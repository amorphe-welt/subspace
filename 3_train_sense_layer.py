...
def gram_schmidt(vectors: np.ndarray) -> np.ndarray:
    """
    Orthonormalize vectors using QR decomposition (more stable than Gram-Schmidt).
    
    Args:
        vectors: Array of shape (n_vectors, dim), where each ROW is a vector
        
    Returns:
        Orthonormalized vectors of shape (n_vectors, dim)
    """
    if len(vectors) == 0:
        return np.array([])
    
    if len(vectors) == 1:
        # Single vector - just normalize
    ...
    # QR decomposition expects columns as vectors, so transpose
    # Input: (n_vectors, dim) -> Transpose to (dim, n_vectors)
    Q, R = np.linalg.qr(vectors.T)
    
    # Q is (dim, n_vectors) with orthonormal columns
    # Transpose back to get (n_vectors, dim) with orthonormal rows
    ortho_vectors = Q.T[:len(vectors)]
    
    # Handle potential sign flips and zero rows from QR
    valid_vectors = []
    for i, vec in enumerate(ortho_vectors):
        ...
        if norm > 1e-10:
            ...
            logging.warning(f"Zero vector {i} after QR - likely linearly dependent input")
    
    if len(valid_vectors) == 0:
        logging.error("All vectors became zero after orthonormalization!")
        return np.array([])
    
    ortho_vectors = np.array(valid_vectors)
    
    # Verify orthonormality
    gram_matrix = ortho_vectors @ ortho_vectors.T
    identity = np.eye(len(ortho_vectors))
    error = np.linalg.norm(gram_matrix - identity)
    
    logging.info(f"  Orthonormality error: {error:.8f}")
    
    if error > 0.01:
        logging.warning(f"  High orthonormality error after QR: {error:.6f}")
        logging.warning(f"  This suggests input vectors are nearly collinear")
    
    
    # DEBUG: Print detailed information
    #print(f"\n=== DEBUG gram_schmidt ===")
    #print(f"Input shape: {vectors.shape}")
    #print(f"Output shape: {ortho_vectors.shape}")
    #print(f"First vector norm: {np.linalg.norm(ortho_vectors[0]):.6f}")
    #if len(ortho_vectors) > 1:
    #    print(f"Dot product of first two vectors: {np.dot(ortho_vectors[0], ortho_vectors[1]):.6f}")
    #print(f"Gram matrix diagonal: {np.diag(gram_matrix)}")
    #print(f"Gram matrix off-diagonal max: {np.max(np.abs(gram_matrix - np.diag(np.diag(gram_matrix)))):.6f}")
    #print(f"Identity check: {np.allclose(gram_matrix, identity, atol=0.01)}")
    #print("=========================\n")
